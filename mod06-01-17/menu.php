	...
	foreach ($items as $index => $item) {
		if($filter == 'All' || $item['category'] == $filter){
			...
			if(isset($_SESSION['username']) && $_SESSION['username'] == 'admin'){
				echo "<button class='btn btn-primary render_modal_body' data-toggle='modal' data-target='#myModal' data-index='$index'>Edit Item</button>";
				echo "<button class='btn btn-danger delete_modal_body' data-toggle='modal' data-target='#myModal' data-index='$index'>Delete Item</button>";
			}
			else 
			if(isset($_SESSION['username'])){
				echo "<form method='post' action='addItemtoCart.php?index=$index'>";
				echo "<input type='number' name='quantity' min=0>";
				echo "<button class='btn btn-success' data-index='$index'>Add to Cart</button>";
				echo "</form>";
			}
			echo "</div>"; 
		}
			  ...
			  <div class="modal-dialog">
			    <div class="modal-content">
			      <div class="modal-header">
			        ...
			        <h4 class="modal-title">Welcome to Tuitt Cafe</h4>
			      </div>
